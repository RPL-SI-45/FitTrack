                                ...
                                @foreach($obats as $obat)
                                    ...
                                    <td class="text-center">
                                        @php
                                          // waktu minum
                                          $satuan_konsumsi = 'day';
                                          if ($obat->satuan_konsumsi === 'minggu') {
                                              $satuan_konsumsi = 'week';
                                          } elseif ($obat->satuan_konsumsi === 'bulan') {
                                              $satuan_konsumsi = 'month';
                                          } elseif ($obat->satuan_konsumsi === 'tahun') {
                                              $satuan_konsumsi = 'year';
                                          }
                                          $waktu_minum = [];

                                          // Waktu minum antara 5 menit sebelum dan 5 menit setelah waktu minum
                                          for($i = -5; $i <= 5; $i++) {
                                              $waktu_minum[] = date('H:i', strtotime($i . ' minutes'));
                                          }

                                          $waktunya_minum = false;
                                          foreach($waktu_minum as $waktu) {
                                              if (in_array($waktu, $obat->jam_minum)) {
                                                  $waktunya_minum = true;
                                                  break;
                                              }
                                          }
                                        @endphp
                                        @if($waktunya_minum && ((date('Y-m-d') >= $obat->tanggal_mulai && date('Y-m-d') <= date('Y-m-d', strtotime($obat->tanggal_mulai . ' + ' . $obat->lama_konsumsi . ' ' . $satuan_konsumsi))) || $satuan_konsumsi == 'seumur_hidup'))
                                            <span class="badge bg-warning">Waktu Minum</span>
                                        @else
                                            <span class="badge bg-secondary">Belum Waktu Minum</span>
                                        @endif
                                    </td>
